...
class TestBindCard(unittest.TestCase):
    ...
    def test_bind_card(self):
        try:
            ...
            chargeWithdrawBankCardPage.login(Constant.NewUser, Constant.Password)

            # Step2:在银行卡管理页面绑卡
            chargeWithdrawBankCardPage.click_my_account()
            chargeWithdrawBankCardPage.click_bank_card_manage_on_left_panel()
            # chargeWithdrawBankCardPage.click_add_bank_card()
            print('开始绑定银行卡......')
            # 填写储蓄卡卡号
            bankCardNum = Constant.CardNumber + str(random.randint(10000000000000, 99999999999999))
            chargeWithdrawBankCardPage.enter_bank_card(bankCardNum)
            chargeWithdrawBankCardPage.click_outside()
            self.assertTrue(chargeWithdrawBankCardPage.get_bank_type() == "中国邮储银行")
            chargeWithdrawBankCardPage.select_province(Constant.Province)
            chargeWithdrawBankCardPage.select_city(Constant.City)
            chargeWithdrawBankCardPage.enter_phone_bank(Constant.MobileInBank)
            # 获取验证码并填写
            clear_logcat()
            chargeWithdrawBankCardPage.click_get_valid_code_in_bind_card()
            smsCode = getsmscode()
            chargeWithdrawBankCardPage.enter_valid_code_in_bind_card(smsCode)
            chargeWithdrawBankCardPage.click_bank_card_submit_button()
            self.assertTrue(chargeWithdrawBankCardPage.get_current_url().__contains__("bind-success"))
            print('绑定银行卡完成......')
